...
def otvori_settings():
    ...
    def label(tekst, row):
        tk.Label(prozor, text=tekst, fg=fg, bg=bg,
                font=("Consolas", 10)).grid(row=row, column=0, padx=20, pady=8, sticky="w")

    def entry(vrednost, row):
        var = tk.StringVar(value=str(vrednost))
        e = tk.Entry(prozor, textvariable=var, bg=entry_bg, fg=fg,
                    font=("Consolas", 10), width=10,
                    insertbackground=fg)
        e.grid(row=row, column=1, padx=20, pady=8)
        return var

    tk.Label(prozor, text="⚙️ PODESAVANJA", fg="#00d4ff", bg=bg,
            font=("Consolas", 12, "bold")).grid(row=0, columnspan=2, pady=10)
    ...
    def sacuvaj():
        try:
            ...
            config.sacuvaj(novi)
            tk.messagebox.showinfo("OK", "Podesavanja sacuvana!\nRestartujte monitor.")
            prozor.destroy()
        except:
            tk.messagebox.showerror("Greska", "Unesite ispravne brojeve!")

    tk.Button(prozor, text="SACUVAJ", command=sacuvaj,
             bg="#00d4ff", fg="#000000",
